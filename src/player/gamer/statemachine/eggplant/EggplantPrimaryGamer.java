...
public class EggplantPrimaryGamer extends StateMachineGamer {
	...
	public void stateMachineMetaGame(long timeout)
		...
		*/
		...
		// (timeout - start) / 2);
		heuristic = new PropNetAnalyticsHeuristic(new Heuristic[] {new NullHeuristic((int)avgGoal)}, new double[] {1.0});
		...
		for (Move move : possibleMoves) {
			...
			for (List<Move> jointMove : jointMoves) {
				...
				if (principalMoveFound) {
					Log.println('d', "NUIDS : At offset " + pvDepthOffset + "; " + nextState);
